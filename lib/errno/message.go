...
var messageMap = map[Errno]*Message{
	...
	TimestampNotFound:        newNoticeMessage("timestamp not found in response", ModuleHeimdall),
	TypeAssertFail:           newNoticeMessage("type assert fail, expect %v", ModuleHeimdall),
	FailToProcessData:        newFatalMessage("fail to process batch point, quantity:%d", ModuleHeimdall),
	FailToFillUpConnPool:     newWarnMessage("fail to fill up connection pool, err:%v", ModuleHeimdall),
	ClientQueueClosed:        newFatalMessage("client queue closed", ModuleHeimdall),
	NoAvailableClient:        newWarnMessage("no available client to send data", ModuleHeimdall),
	ConnectionBroken:         newWarnMessage("found connection broken", ModuleHeimdall),
	ResponseTimeout:          newWarnMessage("data response timeout", ModuleHeimdall),
	FailToConnectToPyworker:  newFatalMessage("fail to connect to heimdall udf", ModuleHeimdall),
	UnknownDataMessage:       newFatalMessage("receive unknown message", ModuleHeimdall),
	UnknownDataMessageType:   newFatalMessage("receive other msgType, expect %s, got:%s", ModuleHeimdall),
	MessageNotFound:          newNoticeMessage("%s not found", ModuleHeimdall),
	InvalidAddr:              newNoticeMessage("not valid pyworker addr, expect ip format", ModuleHeimdall),
	InvalidPort:              newNoticeMessage("port must >= 0", ModuleHeimdall),
	AlgoConfNotFound:         newNoticeMessage("algorithm configuration file not found", ModuleHeimdall),
	AlgoNotFound:             newNoticeMessage("algorithm not found", ModuleHeimdall),
	AlgoTypeNotFound:         newNoticeMessage("algorithm type not found", ModuleHeimdall),
	InvalidResultWaitTimeout: newNoticeMessage("result-wait-timeout must >= 0", ModuleHeimdall),
	InvalidPoolSize:          newNoticeMessage("connect-pool-size must >= 1", ModuleHeimdall),
}
